...
input_dir = '/Users/robert/Documents/UMN/5551_Robots/Project/data/'
output_dir = '/Users/robert/Documents/UMN/5551_Robots/Project/recipes/'

recipe_table = open(output_dir + 'recipes.txt', 'wb')
recipe_table.write('recipe_key\tsentence_key\tsentence\n')
token_table = open(output_dir + 'training.txt', 'wb')
...
for recipe_type in filenames:
	...
	for recipe_key, recipe_text in enumerate(recipes):
		...
		for sentence_key, sentence_text in enumerate(sentences):
			recipe_table.write(str(recipe_key) + '\t' + str(sentence_key) + '\t' + sentence_text + '\n')
			tokens = filter(None, [w.strip() for w in re.split('\s+', sentence_text)])
			for t in range(1, len(tokens)+1):
				token_table_size += 1
				target = 0
				if t == len(tokens):
					target = 1
				token_table.write(str(recipe_key) + '\t' + str(sentence_key) + '\t' + str(t) + '\t' + str(target) + '\t' + ' '.join(tokens[0:t]) + '\n')
